...
public class TestingClass {
	...
	static int totalJudgementParameters = 22;
	...
	public static void main(String args[]) throws Exception {
		...
		List<Double> valueOfparameters = new ArrayList<Double>();
	    ...
	    Cell cellMergeCurrent = rowToMerge.getCell(insertionStartColumn+2);
	    cellMergeCurrent.setCellValue("Current Score");
	    
	    Cell cellMergeExpected = rowToMerge.getCell(insertionStartColumn+2+7+1);
	    cellMergeExpected.setCellValue("Expected Score");
	    
	    Cell cellMergeKPI = rowToMerge.getCell(insertionStartColumn+2+7+1+7+1);
	    ...
	    sheetToCreate.addMergedRegion(new CellRangeAddress(
	    		...
	    		(insertionStartColumn+2), //first column (0-based)
	    		(insertionStartColumn+2+7)  //last column  (0-based)
	    ));
	    ...
	    sheetToCreate.addMergedRegion(new CellRangeAddress(
	    		insertionStartRow, //first row (0-based)
	    		insertionStartRow, //last row  (0-based)
	    		(insertionStartColumn+2+7+1), //first column (0-based)
	    		(insertionStartColumn+2+7+1+7)  //last column  (0-based)
	    ));
	    ...
	    sheetToCreate.addMergedRegion(new CellRangeAddress(
	    		insertionStartRow, //first row (0-based)
	    		insertionStartRow, //last row  (0-based)
	    		(insertionStartColumn+2+7+1+7+1), //first column (0-based)
	    		(insertionStartColumn+2+7+1+7+1+5)  //last column  (0-based)
	    ));
	    ...
	    for(int fetchIndividualProjectScore=0; fetchIndividualProjectScore< excelFiles.size(); fetchIndividualProjectScore++)
		{
	    	...
	    	{
	    		cellToWrite = rowToInsert.createCell(insertionStartColumnCopy++);
	    		cellToWrite.setCellValue(valueOfparameters.get(ok++));
	    		cellToWrite.setCellStyle(style);
	    	}
		}
